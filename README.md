...
本仓库实现了一个用于空战场景的集成专家系统，结合攻击与防御策略，实现对多架友机的协同决策。核心模块现统一收敛到 `airbattle` 包，既包含原有的动态博弈与强化学习实现，也封装了攻击/防御专家系统，便于在单一命名空间下协同验证不同策略组合。为了保证旧代码的可用性，项目根目录仍保留了 `code` 与 `expert` 兼容层，自动转发到新的包路径，无需一次性修改所有历史脚本。
...
2. 运行演示脚本体验攻防协同流程：
   ...
   - 兼容层同样支持旧的调用方式，例如 `python code/demo.py` 或在第三方工程中继续通过 `import code.DroneCombatSystem`、`import expert.expertsystem` 获取原有接口。
